...
String formatDateTime(unsigned long long timestamp_ms);
...
void createCSVFile() {
  ...
  if (dataFile) {
    // Updated CSV header with DateTime in ISO 8601 format
    dataFile.println("DateTime,DataPoint,CAN_Voltage(V),CAN_Current(A),Power(W),MotorTemp(C),ControllerTemp(C),Wheel_RPM_Left,Wheel_RPM_Right,Stroke1_mm,Stroke2_mm");
    dataFile.flush();
    dataFile.close();
...
void logDataToSD() {
  ...
  if (dataFile) {
    // Get synchronized time and format as ISO 8601 with Bangkok timezone
    unsigned long long timestamp_ms = getSynchronizedTime();
    String dateTime = formatDateTime(timestamp_ms);
...
String formatDateTime(unsigned long long timestamp_ms) {
  ...
  unsigned long long timestamp_sec = timestamp_ms / 1000ULL;
  
  // Add Bangkok timezone offset (UTC+7 = 7 * 3600 seconds)
  const long BANGKOK_OFFSET = 7 * 3600;
  timestamp_sec += BANGKOK_OFFSET;
  
  // Calculate date and time components
  unsigned long days = timestamp_sec / 86400ULL;
  unsigned long remaining = timestamp_sec % 86400ULL;
