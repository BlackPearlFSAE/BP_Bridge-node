...
void IMUupdate(Odometry* myimu, Adafruit_BNO055 &bno, bool &AvailableFlag){
  ...
  // Euler: BNO055 register order is x=heading, y=roll(around X/forward), z=pitch(around Y/left)
  // Vehicle convention (per Roll-Pitch-Yaw diagram): roll=around Y-rail, pitch=around X-rail
  // so BNO055 "roll"(y) maps to vehicle pitch, and BNO055 "pitch"(z) maps to vehicle roll
  myimu->imu_euler_yaw   = orientationData.orientation.x;
  myimu->imu_euler_pitch = orientationData.orientation.y;
  myimu->imu_euler_roll  = orientationData.orientation.z;
