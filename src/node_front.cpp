...
void sendToAPI(const char* topic, float value, uint64_t timestamp) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin("https://mctrl.kmutt.ac.th/ken-api/api/stat/add");
...
void append_BAMOdata_toCSVFile(File& dataFile, void* data) {
  ...
  dataFile.print(",");
  dataFile.println(b->controllerTempValid ? b->controllerTemp : 0.0, 1);
}
