...
void setup() {
  ...
  // Initialize CAN Bus
  initCANBus(CAN_TX_PIN,CAN_RX_PIN,canBusReady,TWAI_TIMING_CONFIG_250KBITS());
  // if (canBusReady) {
  //   delay(3000);  // Wait for Bamocar to be ready
  //   scanBamocarIDs();
  // }
  ...
  if (canBusReady) {
    ...
    if (millis() - lastCANRequest >= CAN_REQUEST_INTERVAL) {
      switch (canRequestState) {
        case 0:
          // The requestBamocarData will be passed through the sedn and requece
...
// make this a callback for receive polling
