...
// const char* WIFI_PASSWORD = "p6xj2xaw";

// NTP Configuration (SNTP server for pulling timestamp)
 ...
 * ============================================== */
// #define RPM_CALCULATION_PERIOD_MS 100  // 100ms RPM calculation period
 
// #define RPM_MS_PER_MIN 60000.0
// #define FLOAT_ENCODE_FACTOR 100

#define RCCHECK(fn) { rcl_ret_t temp_rc = fn; if((temp_rc != RCL_RET_OK)){ error_loop();} }
#define RCSOFTCHECK(fn) { rcl_ret_t temp_rc = fn; if((temp_rc != RCL_RET_OK)){} }
#define TIMER_PUBLISH_PERIOD_MS 200   //
...
// Define 4 timers and Publisher objects
rcl_timer_t timer_1;
rcl_timer_t timer_2;
rcl_publisher_t pub_1;
rcl_publisher_t pub_2;
byte topicNum = 1;
// message will be defined in call back
...
void error_loop();
void flushBufferToUROS(rcl_timer_t * timer, int64_t last_call_time);

void initializeSystem(bool virtualCANmode = false, bool disableSD = false);
void initSD();
void initTWAI();
void initWiFi();
void syncTime();
...
void setup() {
  Serial.begin(115200);
  USBSerial.begin(115200); // Or higher
  ...
  bool virtualCANmode = true;
  bool disableSD = true;
  initializeSystem(virtualCANmode);
  
  Serial.println("\n=== BAMOCAR LOGGER COMMANDS ===");
  // Might add a very specific commmand to toggle disable SD logging entirely (Used for data flow test)
  ...
  Serial.println("'M' - Toggle on/off SD logging mode");
  ...
  // Create node object
  RCCHECK(rclc_node_init_default(&node, "BAMOnode", "", &support));
  ...
  // create timer
    ...
    &timer_1,
    &support,
    RCL_MS_TO_NS(TIMER_PUBLISH_PERIOD_MS),
    flushBufferToUROS));

  RCCHECK(rclc_executor_init(&executor, &support.context, topicNum, &allocator));
...
void loop() {
  ...
  // ---------- UROS LOGGING  
  RCSOFTCHECK(rclc_executor_spin_some(&executor, RCL_MS_TO_NS(TIMER_PUBLISH_PERIOD_MS)));
  ...
  if (millis() - lastLogTime >= LOG_INTERVAL_MS) {
    ...
    // Add to buffer if logging command is pressed
    if (isLogging) {
      addDataToBuffer();
...
void initializeSystem(bool virtualCANmode , bool disableSD) {
  ...
  if(!disableSD)
    ...
    Serial.println("SD is disabled");
  // Initialize TWAI CAN system if the flag is not true
  if(!virtualCANmode)
    initTWAI();
  else
    Serial.println("TWAI is disabled");
...
void startLogging() {
  ...
  // Catch the condition if isLogging is already true , toggle stop
  if (isLogging) {
    stopLogging();
    delay(200);
  }
...
// Timer callback - executes Suspension stroke calculation and publishing
void flushBufferToUROS(rcl_timer_t * timer, int64_t last_call_time) {  
  ...
  if (timer != NULL) {
    msg.data.data[0] = dataBuffer[0].voltage;
    msg.data.data[1] = dataBuffer[0].current;
    msg.data.data[2] = dataBuffer[0].power;
    msg.data.data[3] = dataBuffer[0].motorTemp;
    msg.data.data[4] = dataBuffer[0].bamoTemp;
    // add more
    RCSOFTCHECK(rcl_publish(&pub_1, &msg, NULL));
  }
    // Clean up
    free(msg.data.data);
}
